...
Complete manifests for deploying the orbital tracker on Kubernetes. Production runs on EC2 rather than EKS because the EKS control plane costs ~$73/month regardless of workload, not worth it for a project at this scale. The manifests are here for portability and were validated end-to-end on minikube locally and a Civo cloud cluster.
...
# (imagePullPolicy: Never means the cluster won't try to pull from a registry)
